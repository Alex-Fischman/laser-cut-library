/* To use this:
- change THICKNESS to the thickness of your material, accounting for kerf
- use piece() to generate pieces and add them to an array after rotate()/translate()/etc them
-- piece() takes an object containing:
--- path: CSG.Path2D instance to bound the piece
--- slots: optional array of 7-tuples, each of which is [length, rX, rY, rZ, x, y, z]
describing a slot cut out of the piece, with the given length, rotated by [rX,rY,rZ] and then translated by [x,y,z]
- use layoutForPrint() to generate a print
-- layoutForPrint() takes 2 arguments:
--- a boolean indicating whether or not to pose in 3D or 2D
--- an array containing all the pieces you want to cut
*/
...
axes = function() {
	function arrow(name) {
		...
		return union(cylinder({r1: length/10, r2: 0, h: length}).rotateY(90), text(name)).setColor([255, 0, 0]);
	}
	var offset = 90;
	return union(
